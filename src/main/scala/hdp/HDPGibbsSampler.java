...
public class HDPGibbsSampler implements Serializable{
	...
	protected int[] wordCountByTopic;//nk
	protected int[][] wordCountByTopicAndTerm;//nkv
	 ...
	 * 
	 * @paramcorpus {@linkCLDACorpus} on which to fit the model
	 */
	public void addInstances(int[][] documentsInput, int V) {
		...
		System.out.println("numberOfTables = " + totalNumberOfTables);
		int k, i, j;
		DOCState docState;
		p = new double[20];
		f = new double[20];
		...
		for (j = numberOfTopics; j < docStates.length; j++) {
			...
			k = random.nextInt(numberOfTopics + 1);
			System.out.println("instance k : " + k);
			for (i = 0; i < docState.documentLength; i++) 
				addWord(docState.docID, i, 0, k);
			System.out.println("instance numberOfTopic : " + numberOfTopics);
	...
	protected void removeWord(int docID, int i){
		...
		if (docState.wordCountByTable[table] == 0) { // table is removed
			totalNumberOfTables--; //(tmpTotalTable, -1)
			numberOfTablesByTopic[k]--; //(topic, tmpTable, -1)
	...
	protected void addWord(int docID, int i, int table, int k) {
		...
		if (docState.wordCountByTable[table] == 1) { // a new table is created
			System.out.println("addWord totalNumberOfTables: " + totalNumberOfTables);
			...
			System.out.println("addWord numberOfTopics: " + numberOfTopics + " k : " + k);
			if (k == numberOfTopics) { // a new topic is created
				System.out.println("addWord numberOfTopics: " + numberOfTopics);
				numberOfTopics++; 
	...
	public static void main(String[] args) throws IOException {
		...
		CLDACorpus corpus = new CLDACorpus(new FileInputStream("sampledata.txt"));
		...
		hdp.run(0, 100, System.out);

		PrintStream file = new PrintStream("coe.txt");
		...
		file = new PrintStream("result.txt");
